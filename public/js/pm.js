...
$(document).ready(function () {
  ...
  function displayProjectPhases(data) {
    phaseArray = [];
    projectPhases = data;
    projectPhases.forEach((phase) => {
      $.get("/api/tasks/phase/" + phase.id).then((res) => {
        var taskArrayText = "";
        for (let i = 0; i < res.length; i++) {
          taskArrayText += `
                   <div class="card task-card" value="${res[i].ProjectPhaseId} style="width:auto">
                      <div class="card-body">
                              <h5>${res[i].taskname}</h5>
                              <ul>
                                  <li>${res[i].UserId}</li>
                                  <li>${res[i].isComplete}</li>
                              </ul>
                       </div>
                   </div>`;
        }
  ...
  function fetchArray(arrayName, id_ref) {
    switch (arrayName) {
      case "userProjects":
        $.get(`/api/projects/${id_ref}`).then((data) => {
          displayUserProjectsNav(data);
        });
        break;
      case "projectPhases":
        projectDisplayArea.empty();
        $.get(`/api/project-phase/project_id=${id_ref}`).then((data) => {
          currentProjectId = id_ref;
          displayProjectPhases(data);
        });
        break;
      case "phaseTasks":
  ...
  function createPhase(phaseData) {
    $.post("/api/project-phase", phaseData).then(()=>{loadProject(currentProjectId)});
  }
